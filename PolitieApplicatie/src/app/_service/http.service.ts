    ...
    return new Promise(resolve => {
      this.get('/politie/vehicles')
        .subscribe(data => {
          const vehicles = [];
          data.json().forEach(v => {
            const newVehicle = new Vehicle(v.id, v.cartrackerHardware, v.licensePlate, v.vehicleType);
  ...
  findVehicleById(vehiclesId: Number, options?: Headers): Promise<Vehicle> {
    return new Promise(resolve => {
      this.get('/politie/vehicles/' + vehiclesId)
        .subscribe(data => {
          console.log(data);
          resolve(new Vehicle(data.json().id, data.json().cartrackerHardware, data.json().licensePlate, data.json().vehicleType));
        }, error => {
          this.handleError(error);
          resolve(null);
        });
